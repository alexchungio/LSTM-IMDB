...
def main(argv):
    ...
    test_dataset =  dataset_batch(x_test, y_test, batch_size=cfgs.BATCH_SIZE, is_training=False)
    ...
    with tf.Session(config=config) as sess:
        ...
        test_step_pre_epoch = num_test_samples // cfgs.BATCH_SIZE

        train_data_batch, train_label_batch = train_dataset.get_next()
        test_data_batch, test_label_batch = test_dataset.get_next()
        for epoch in range(1, cfgs.NUM_EPOCH+1):
            ...
            for step in range(test_step_pre_epoch):
                x_test, y_test = sess.run([test_data_batch, test_label_batch])
